...
const processRequest = async (req, res) => {
  ...
  try {
    ...
    const recommendationResponse = await axios.post(
      `${BACKEND2_URL}/get-recommendations`,
      {
    ...
    if (req.file) {
      try {
        ...
        console.log(
          "✅ Successfully deleted uploaded image file:",
          req.file.filename
        );
      } catch (cleanupError) {
        console.warn(
          "⚠️ Failed to delete uploaded file (non-critical):",
    ...
    // Clean up uploaded file if processing failed (success case is handled above)
    if (req.file && savedRecord?.status === "failed") {
      try {
        if (fs.existsSync(req.file.path)) {
          fs.unlinkSync(req.file.path);
          console.log(
            "🧹 Cleaned up failed processing file:",
            req.file.filename
          );
        }
      } catch (cleanupError) {
        console.error(
          "❌ Failed to cleanup failed processing file:",
          cleanupError.message
        );
      }
    }
...
const getUserHistory = async (req, res) => {
  try {
    ...
    const options = {
      ...
      sort: { createdAt: -1 },
      select:
        "topPrediction aiRecommendation status createdAt originalFilename processingTime",
    };
...
const getUserStats = async (req, res) => {
  try {
    ...
    const recentPredictions = await DiseaseDetection.getRecentPredictions(
      userId,
      5
    );
    ...
    const avgConfidence =
      stats.find((s) => s._id === "completed")?.avgConfidence || 0;
    ...
    res.status(200).json({
      ...
      data: {
        ...
        successRate:
          totalDetections > 0
            ? ((successfulDetections / totalDetections) * 100).toFixed(1)
            : 0,
